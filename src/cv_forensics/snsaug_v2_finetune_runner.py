...
def validate_snsaug_v2_finetune_smoke_config(raw: dict[str, Any], require_exists: bool = False) -> list[str]:
    ...
    if isinstance(max_steps, bool) or not isinstance(max_steps, int) or not (1 <= max_steps <= 500):
        errors.append("max_steps must be an integer in 1..500")
...
def run_snsaug_v2_finetune_smoke(config: dict[str, Any], *, dry_run: bool = False) -> dict[str, Any]:
    ...
    log_path = _write_jsonl(
        output_root / "smoke_train_log.jsonl",
        [
            {
                "marker": MARKER,
                "event": "guarded_smoke_setup",
                "step": 0,
                "max_steps": int(config["max_steps"]),
                "training_started": False,
                "note": "Unit-safe path materialized smoke artifacts; approved optimization must be run explicitly on lab data.",
            }
        ],
    )
    summary_payload = {
        "marker": MARKER,
        "training_started": False,
        "status": "guarded_smoke_runner_ready",
        "model_name": "SNSAug-aware Multi-head Forensics Model v1",
        "model_version": MODEL_VERSION,
        "max_steps": int(config["max_steps"]),
        "epochs": int(config["epochs"]),
        "note": "Real optimization is intentionally guarded; this path verifies smoke artifact/checkpoint routing.",
        "plan": plan,
    }
    summary_path = _write_json(output_root / "smoke_train_summary.json", summary_payload)
    loss_path = _write_json(
        output_root / "loss_breakdown.json",
        {
            "marker": MARKER,
            "loss_terms": plan["losses"],
    ...
    clean_eval_path = _write_json(
        output_root / "smoke_eval_clean_summary.json",
        {
            ...
            "status": "pending_real_smoke_execution",
        },
    )
    ...
    checkpoint_path = _write_json(
        checkpoint_root / "snsaug_aware_multihead_forensics_v1_smoke_checkpoint_manifest.json",
        {
            "marker": MARKER,
            "model_version": MODEL_VERSION,
            "checkpoint_kind": "guarded_smoke_checkpoint_manifest",
            "base_model_bundle_path": config["base_model_bundle_path"],
            "trainable_components": plan["trainable_components"],
            "training_started": False,
            "note": "Placeholder manifest verifies checkpoint routing; approved lab smoke optimization should replace this with model weights.",
        },
    )
    output_paths = {
